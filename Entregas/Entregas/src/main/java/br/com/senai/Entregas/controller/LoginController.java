...
import br.com.senai.Entregas.dtos.LoginRequest;
...
public class LoginController {
    // 1. Injetamos os "motores" que configuramos no SecurityConfigution
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest){
        // pega o corpo json da requisição e converte para um objeto LoginRequest

        // AUTENTICAÇÃO
        // 2. Cria um pedido de autenticação com o email e senha do usuário
        var authToken = new UsernamePasswordAuthenticationToken(loginRequest.getEmail(), loginRequest.getSenha());

        // 3. Entrega o pedido ao gerente
        // Caso a autenticação falhe, uma exceção é lançada e o método para aqui
        Authentication auth = authenticationManager.authenticate(authToken);

        // pega o momento atual para pegar o momento de criação do token e de
        // expiração do mesmo
